...
import com.wb.workflow.core.cmd.request.instance.WorkFlowSuspendInstanceCmdRequest;
...
/**
 * @ClassName: WorkFlowSuspendInstanceCmd
 * @Package: com.wb.workflow.core.cmd.instance
 * @Description: the work flow suspend instance cmd
 * @Author：[bo.wang]
 * @Date: 2021/3/1
 * @Version: 1.0
 */
@Component
public class WorkFlowSuspendInstanceCmd extends AbstractWorkFlowInstanceCmd {
    ...
    public void check(WorkFlowGenericCmdRequest cmdRequest) {
        ...
        if (WorkFlowInstanceStatusEnum.SUSPEND.name().equalsIgnoreCase(status)) {
    ...
    public <T> T execute(WorkFlowGenericCmdRequest<T> cmdRequest) {
        WorkFlowInstanceEntity instanceEntity = cmdRequest.getInstance();
        instanceExt.suspend(instanceEntity.getfInstanceId());
        instanceEntity.setStatus(WorkFlowInstanceStatusEnum.SUSPEND.name());
    ...
    public Class<?> getReqClass() {
        return WorkFlowSuspendInstanceCmdRequest.class;
    }
}
